...
class Bug1Planner:
    ...
    def check_bf(self, leave_point, current_pos, path):
        ...
        For Bug1 algorithm this condition is verified when the current position is equals to hit point.

        :return: False if hit point has been reached
        """
        i = path.__len__() - 1
        if i > 0:
            dist = math.sqrt((self.end.x - path[i].x) ** 2 + (self.end.y - path[i].y) ** 2)
            min_dist = (path[i], dist)
        while i >= 0:
            if path[i].type == BF:
                dist = math.sqrt((self.end.x - path[i].x) ** 2 + (self.end.y - path[i].y) ** 2)
                if min_dist[1] > dist:
                    min_dist = (path[i], dist)
                if path[i - 1].type == M2G:
                    if current_pos.x == path[i].x and current_pos.y == path[i].y:
                        j = path.index(min_dist[0])
                        max_len = len(path) - 1
                        if j - i <= max_len - j:
                            while i <= j:
                                path.append(Point(path[i].x, path[i].y, BF2LP))
                                i += 1
                        else:
                            max_len -= 1
                            while j <= max_len:
                                path.append(Point(path[max_len].x, path[max_len].y, BF2LP))
                                max_len -= 1
                        current_pos.x, current_pos.y = min_dist[0].x, min_dist[0].y
